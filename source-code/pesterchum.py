...
class chumArea(RightClickTree):
    ...
    def editNotes(self):
        ...
        (notes, ok) = QtWidgets.QInputDialog.getText(
            self, "Заметки", "Введи свои заметки..."
        )
        if ok:
    ...
    def renameGroup(self):
        ...
        if not self.renamegroupdialog:
            (gname, ok) = QtWidgets.QInputDialog.getText(
                self, "Переименовать группу", "Введите новое имя для группы:"
            )
            if ok:
                if re.search(r"[^A-Za-z0-9_\s]", gname) is not None:
                    ...
                    msgbox.setInformativeText("ЭТО НЕКОРРЕКТНОЕ ИМЯ ДЛЯ ГРУППЫ")
    ...
    def addTrollWindow(self):
        ...
        if ok:
            ...
            if not (PesterProfile.checkLength(handle)):
                ...
                errormsg.showMessage("ЭТО НЕКОРРЕКТНЫЙ ЧАМТЕГ!")
...
class PesterWindow(MovingWindow):
    ...
    def __init__(self, options, parent=None, app=None):
        ...
        try:
            ...
            msgBox.setText(
                "<html><h3>Ошибка профиля, "
                "пробуем сбросить на стандартный профиль"
                r"<br><br>%s<\h3><\html>" % e
            )
            PchumLog.critical(e)
            msgBox.exec()
        ...
        try:
            ...
            self.reportBugAction = QAction("СООБЩИТЬ О БАГЕ", self)
        try:
            self.xyzRulesAction = QAction(self.theme["main/menus/help/rules"], self)
        except:
            self.xyzRulesAction = QAction("ПРАВИЛА", self)
        ...
        if self.config.updatecheck():
            ...
            PchumLog.info("Проверка обновлений отключена, пропускаем...")
    ...
    def root_check(self):
        ...
        if ostools.isRoot() or ostools.isAdmin():
            ...
            msg = (
                ...
                "\nTНет веских причин запускать это от root."
                "\n\nВыйдем?"
            )
            msgbox.setWindowTitle("Предупреждение о ненужных разрешениях")
            msgbox.setStyleSheet(
            ...
            continue_anyway.setText(
                "Я малеьнкий человек и всё-равно хочу продолжить"
            )
    ...
    def checkPing(self):
        ...
        if self.sincerecv >= 45:
            if not self.parent.irc.unresponsive:
                ...
                self.parent.showLoading(self.parent.widget, "С3Р83Р НОУ Р3СПOНДИНГ >:?")
                self.show()
                self.setFocus()
        else:
    ...
    def closeToTray(self):
        ...
        t = self.tm.Toast("Уведомление:", "Pesterchum свёрнут в трей.")
        t.show()
        self.hide()
        self.closeToTraySignal.emit()

    def closeEvent(self, event):
        if hasattr(self, "троллслум") and self.trollslum:
    ...
    def newMessage(self, handle, msg):
        if handle in self.config.getBlocklist():
            ...
            if not self.config.irc_compatibility_mode():
                self.sendMessage.emit("ПЕСТЕРЧАМ:БЛОКЕД", handle)
            return
        # notify
        if self.config.notifyOptions() & self.config.NEWMSG:
            if handle not in self.convos:
                t = self.tm.Toast("Новый диалог", "From: %s" % handle)
                t.show()
                ...
                if msg[:11] != "ПЕСТЕРЧАМ:":
                    ...
                    if msg == "ПЕСТЕРЧАМ:ПРЕКРАТИТЬ":
                        t = self.tm.Toast("Закрыть диалог", handle)
                        t.show()
                    elif msg == "ПЕСТЕРЧАМ:БЛОКНУТЬ":
                        t = self.tm.Toast("Блокнуть", handle)
                        t.show()
                    elif msg == "ПЕСТЕРЧАМ:АНБЛОКНУТЬ":
                        t = self.tm.Toast("Анблокнуть", handle)
                        t.show()
        if handle not in self.convos:
            if msg == "ПЕСТЕРЧАМ:ПРЕКРАТИТЬ":  # ignore cease after we hang up
                return
        ...
        if self.config.soundOn():
            if self.config.chatSound() or convo.always_beep:
                if msg in ["ПЕСТЕРЧАМ:ПРЕКРАТИТЬ", "ПЕСТЕРЧАМ:БЛОКНУТЬ"] and self.ceasesound:
    ...
    def initTheme(self, theme):
        ...
        try:
            ...
            self.reportBugAction.setText("РЕПОРТ ОБ ОШИБКЕ")
    ...
    def deliverInvite(self, handle, channel):
        ...
        msgbox.setInformativeText(
            ("%s пригласил вас присоединиться к мемо: %s" "\nХотите присоединиться к ним?")
            % (handle, channel)
        )
        msgbox.setStandardButtons(
        ...
        if not self.addchumdialog:
            ...
            if ok:
                ...
                if re.search(r"[^A-Za-z0-9_\s]", group) is not None:
                    errormsg = QtWidgets.QErrorMessage(self)
                    errormsg.showMessage("ЭТО НЕКОРРЕКТНОЕ ИМЯ ГРУППЫ")
                    self.addchumdialog = None
                    return
                if newgroup:
                    # make new group
    ...
    def reportChum(self, handle):
        (reason, ok) = QtWidgets.QInputDialog.getText(
            self,
            "Репорт юзеру",
            "Введите причину репорта данного юзера:",
        )
        if ok and reason:
            ...
            msgbox = QtWidgets.QMessageBox()
            msgbox.setStandardButtons(QtWidgets.QMessageBox.StandardButton.Ok)
            msgbox.setInformativeText("Пожалуйста, укажите причину.")
            msgbox.exec()
    ...
    def switchProfile(self):
        if self.convos:
            ...
            closeWarning.setText(
                "АЛЯРМА: СМЕНА ПРОФИЛЯ ЗАКРОЕТ ВСЕ ДИАЛОГИ"
            )
            closeWarning.setInformativeText(
                "Я ТЕБЯ ПРЕДУПРЕЖДАЛ НАСЧЁТ ОКОШЕК БРАТАН!!!! Я ТЕБЕ ГОВОРИЛ!"
            )
    ...
    def tooManyPeeps(self):
        ...
        msg.setText("D: ТУУУ МЭНИ ПИПЛ!!!")
        msg.setInformativeText(
            "Сервер достиг максимума! Грустненько конечно, да, но подумайте - если вы это видите, вы, вероятно, первый человек за много лет, кто это увидел. УРА!"
        )
    ...
    def resetServerlist(self):
        ...
        if os.path.isfile(_datadir + "serverlist.json"):
            ...
            msgbox.setWindowIcon(PesterIcon(self.theme["main/icon"]))
            msgbox.setInformativeText(
                "Не подтянул список серверов. Вернёмся к настройками по умолачнию?\n"
                "Если вы выберете нет, пестер, скорее всего, жёстко зависнет пока вручную не поправите файлик serverlist.json\n"
                "Плез, сообщи мне если возникнет эта ошибка снова :'3"
            )
            msgbox.addButton(
            ...
            PchumLog.warning(
                "Фейл. Файл serverlist.json не существует, ого "
                "Ноу проблем, если пестер заводишь впервые на этом кампукцтере."
            )
            with open(_datadir + "serverlist.json", "w") as server_file:
                server_file.write(json.dumps(default_server_list, indent=4))
                server_file.flush()
    ...
    def setServer(self):
        if self.serverBox.currentText() == "Адд сервер[Prompt]":
            # Text input
            ...
            # Buttons
            cancel = QtWidgets.QPushButton("КАНЦЕЛ")
            ok = QtWidgets.QPushButton("ОК")
            ...
            TLS_checkbox_label = QtWidgets.QLabel(
                ":33 < Нажми эту галочку если подключаешься по TLS!!"
            )
            ...
            nep_prompt = QtWidgets.QLabel(
                ":33 < Плиз пут сервер нейм ин формат HOSTNAME:PORT\n:33 < Для примера, irc.pesterchum.xyz:6697"
            )
            nep_prompt.setStyleSheet("QLabel { color: #416600; font-weight: bold;}")

            auth_pass_prompt = QtWidgets.QLabel(":33 < введи пароль!! (optional)")
            ...
            layout.addWidget(ok)
            main_layout = QtWidgets.QVBoxLayout()
            main_layout.addWidget(QtWidgets.QLabel("Выбери сервер для удаления."))
            ...
            try:
                ...
                PchumLog.error("Фейл ту настройки сервера :(")
    ...
    def chooseServer(self):
        ...
        layout.addWidget(ok)
        main_layout = QtWidgets.QVBoxLayout()
        main_layout.addWidget(QtWidgets.QLabel("Плиз чууз а сервер."))
    ...
    def connectAnyway(self, e):
        ...
        try:
            ...
            pass
        msgbox.setIcon(QtWidgets.QMessageBox.Icon.Warning)
        msgbox.setText("Сервер сертификат валидейшн файлед")
        msgbox.setInformativeText(
...
class MainProgram(QtCore.QObject):
    def __init__(self):
        ...
        self.app.setApplicationName("Пестерчам")
        ...
        for folder in ["smilies", "themes"]:
            if not os.path.isdir(folder):
                msgbox = QtWidgets.QMessageBox()
                msg = (
                    "'%s' папка не найдена, Pesterchum"
                    "скорее всего не будет работать корректно."
                    "\nПроверь распаковку архива, "
                    "либо переустанови клиент." % folder
                )
                msgbox.setWindowTitle("СКИЛЛ ИССУЕ >:[")
                msgbox.setInformativeText(msg)
        ...
        moodMenu = self.traymenu.addMenu("СЕТ НАСТРОЕНИЕ")
        moodCategories = {}
        ...
        miniAction = QAction("МИНИМАЙ3", self)
        miniAction.triggered.connect(self.widget.showMinimized)
        exitAction = QAction("ВЫХ0Д", self)
        exitAction.triggered.connect(
    ...
    def showLoading(self, widget, msg="К0НН3КТ1НГ"):
        self.widget.show()
        if len(msg) > 60:
            newmsg = []
            while len(msg) > 60:
    ...
    def restartIRC(self, verify_hostname=True):
        ...
        if stop is None:
            ...
            if self.attempts == 1:
                ...
                msg = "Р3КОННЕКТ1НГ %d" % (self.attempts)
            else:
                msg = "КОННЕКТ1НГ"
            ...
            self.showLoading(self.widget, "Ф41Л3Д: %s" % stop)
    ...
    def uncaughtException(self, exc, value, tb):
        ...
        try:
            ...
            msgbox.setText(
                "Произошло необработанное исключение: %s \n%s \n%s "
                % (exc, value, "".join(traceback.format_tb(tb)))
            )
            msgbox.exec()
        except Exception as e:
            print(f"Не удалось обработать данные, за исключением: {e}")
...
class UpdateAvailable(QtWidgets.QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.new_version_title = QtWidgets.QLabel(
            "Новая версия пестерчама АВАИЛЕЙБЛ!"
        )
        self.const_currentversion = QtWidgets.QLabel("Куррент версион:")
        self.const_latestversion = QtWidgets.QLabel("Латест версион:")
        self.var_currentversion = QtWidgets.QLabel()
        self.var_latestversion = QtWidgets.QLabel()
        self.acceptUpdate = QtWidgets.QPushButton("Гет латест версион!", self)
        ...
        self.changelogTitle = QtWidgets.QLabel("Чейнджлог:")
        self.changelogContents = QtWidgets.QLabel()
        self.postpone = QtWidgets.QPushButton("Напомни мне позже", self)
